...
public class HttpUtils {
	...
	private static final HttpHost PROXY_HOST = new HttpHost("127.0.0.1", 8888,
			"http");
	private static RequestConfig DEFAULT_REQUEST_CONFIG = RequestConfig
			.custom().setCookieSpec(CookieSpecs.BROWSER_COMPATIBILITY).build();
	...
	private final static SSLConnectionSocketFactory DEFAULT_SSLSF = new SSLConnectionSocketFactory(
			sslContext);
	...
	public static CloseableHttpClient getHttpClient(
			SSLConnectionSocketFactory sslcsf, CookieStore cookieStore) {
	...
	public static void executeGet(CloseableHttpClient client, String url)
